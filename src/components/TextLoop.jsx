...
const TextLoop = ({
  ...
  const [metrics, setMetrics] = useState({ length: 0, reps: 1, actualWidth: 0 });
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const reps = unitWidth > 0 ? Math.ceil(length / unitWidth) : 1;
      const actualWidth = unitWidth * reps;
      
      setMetrics(prev => (prev.length === length && prev.reps === reps && prev.actualWidth === actualWidth ? prev : { length, reps, actualWidth }));
    };
  ...
  useEffect(() => {
    const { length, actualWidth } = metrics;
    const head = headRef.current;
    const tail = tailRef.current;
    if (!head || !tail || !length || !actualWidth) return undefined;

    const apply = offset => {
      const partner = offset >= 0 ? offset - actualWidth : offset + actualWidth;
    ...
    const tween = gsap.to(state, {
      offset: direction === 'reverse' ? -actualWidth : actualWidth,
      duration: actualWidth / speed,
      ease: 'none',
      repeat: -1,
